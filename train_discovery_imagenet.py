...
def train_object_discovery(args,trainloader,valloader,model):
    ...
    while True:
        ...
        with torch.no_grad():
            ...
            print('====> Val | Step: {} \t Loss = {:5f}'.format(total_iter, val_loss))
...
def train_prediction(args,model):
    train_epoch_size = len(trainloader)
    if args.exp_dis:
        resume_path=os.path.join(args.log_path,"checkpoints",str(args.dataset)+"_dis.pth")
    ...
    while True:
        ...
        for batch_idx, sample in enumerate(trainloader, 0):
            ...
            if total_iter>=args.cls_steps:
                break
        if epoch>=100:
            evaluator.openmax_processor(trainloader,testloader,outloader,model)
        epoch+=1
        total_loss /= train_epoch_size
...
if __name__=="__main__":
    ...
    parser.add_argument('--dataset', default="tinyimagenet", type=str, help='random seed')
    parser.add_argument('--exp_name', default="tiny", type=str, help='name of experiment')
    parser.add_argument('--batch_size', default=64, type=int)
    parser.add_argument('--image_size', default=64, type=tuple)
    ...
    if args.dis_exp:
        train_object_discovery(args,trainloader,testloader,dis_model)
