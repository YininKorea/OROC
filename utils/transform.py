...
class Crop(object):
    ...
    def __call__(self, sample):
        ...
        salience=salience.resize((w,h),resample=Image.NEAREST)
        ...
        if pad_w or pad_h:
            ...
            salience=ImageOps.expand(salience,border=border,fill=0)
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        salience=salience.crop((start_x,start_y,start_x+self.crop_width,start_y+self.crop_height))
...
class HorizontalFilp(object):
    def __call__(self,sample):
        ...
        if random.random()<0.5:
            ...
            salience=salience.transpose(Image.FLIP_LEFT_RIGHT)
...
class RandomRotation(object):
    ...
    def __call__(self, sample):
        ...
        salience = salience.rotate(rotate_degree, Image.NEAREST)
...
class ToTensor(object):
    def __call__(self, sample):
        ...
        salience=np.asarray(salience).copy()
        img=torch.from_numpy(img).float()
        label=torch.from_numpy(label).long()
        salience=torch.from_numpy(salience).long()
        sample={
            'img':img,
            'label':label,
            'salience': salience
