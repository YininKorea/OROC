...
def create_val_img_folder(root="D:\\datasets\\openood\\data\\image_classic\\tiny_imagenet"):
    '''
...
class TinyImageNet(torchvision.datasets.ImageFolder):
    ...
    def __init__(self, root, transform,split):
        ...
        super(TinyImageNet, self).__init__(root, transform,split)
        self.uq_idxs = np.array(range(len(self)))
        self.max_num_object=3
        self.nbr_classes=20
        self.split=split
...
def get_tiny_image_net_datasets(train_transform, test_transform, train_classes=range(20),
    ...
    train_dataset_whole = TinyImageNet(root=tin_train_root_dir, transform=train_transform,split='train')
    ...
    test_dataset_known = TinyImageNet(root=tin_val_root_dir, transform=test_transform,split='val')
    test_dataset_known = subsample_classes(test_dataset_known, include_classes=train_classes)
    #print(len(test_dataset_known))
    # Get testset for unknown classes
    test_dataset_unknown = TinyImageNet(root=tin_val_root_dir, transform=test_transform,split='test')
    #print(len(test_dataset_unknown))
...
if __name__ == '__main__':
    ...
    # x = get_tiny_image_net_datasets(train_transform=None, test_transform=None, train_classes=range(20),
    #                        open_set_classes=range(20,200),balance_open_set_eval=False,seed=0)
